...
The `complex_pow` is the just double and add of repeated multiplication.
...
```
If we compare it normal RSA, all that's left to do is compute the group order, ie the totient $\phi(n)$ and take it's modular inverse wrt $e$ to get the private key. Once we have the private key, the same `complex_pow` should do the decryption. Well so what is this totient when Gaussian Integers are involved?
...
If you are interested in more of the math behind this, you can check out [rkm's blog](https://rkm0959.tistory.com/192). He has a very good writeup with lot's of different ways to solve this.
